...
# %%  Stability cross-validation
    ...
    if explore_stability_cross_val:
        ...
        predictions_perturbed.loc[:,'Prediction']  = prob_to_label(predictions_perturbed.loc[:, tti.pred_col_name].values, th_min, th_max)    
...
# See description of tvt.create_error_as_feature_model() for details
    ...
    ###############################################################################
    # The function is based on validation rather than cross-validation
    ...
    # num_models = len(tti.fitted_models)
    
    # performance_list = []
    
    # for i in range(num_real):
    #     # print(i)
    #     random_noise = np.random.uniform(-0.5, 0.5, size=(l, m)) * noise
    #     X_random[cols] = X_random[cols] + random_noise
        
    #     y_set_list = []
    #     for ind in range(num_models):
    #         X_ind = X_random[X_random.Set_id == ind]
    #         prediction_set = tti.predict(X_ind, ind, **kwargs)
    ...
    #     tti.set_dic['noise_prediction'] =  X_y_prediction
        
    #     performance_list.append(tti.get_performance(input_set='noise_prediction', verbose=0,  **kwargs))    
